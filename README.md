...
Lucode 是一个中文优先、项目本地优先的终端代码代理。它的核心特点是把多模型调度、多 Agent 执行、Skill 能力、MCP 工具、CLI 快速路径和 JSONL 会话记忆合成一个可配置的开发工作台：用户可以在任意项目目录中运行 `lucode`，让它读取当前 `.lucode` 工作区，按任务复杂度选择单 Agent、串行多 Agent 或并行多 Agent，并通过中文命令菜单完成模型连接、脑位切换、安全审批、会话恢复和工具调用。

V1.2 的重点不是堆功能，而是把真实开发时最常见的流程做成闭环：Provider 连接向导负责接入云端或本地模型，多脑模型调音台负责给前置优化脑、主脑、执行脑和汇总脑分别选模型，能力探测负责判断模型是否适合工具调用和规划任务，命令安全分析负责拦截危险操作，只读 fast path 则优先用本地 CLI 完成状态、diff 和配置摘要，减少无谓的 token 消耗。

> 当前 V1.2 是产品化推进版本：核心链路已经可用，适合个人项目和小范围测试。后续仍会继续完善原生 Provider、错误恢复、沙箱隔离和公开发布文档。
