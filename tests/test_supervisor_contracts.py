...
class FullSupervisorObserverTests(unittest.TestCase):
    ...
    def test_validator_allows_supervised_full_team_without_summary_helper(self):
        ...
        with patch("planning.plan_validator.load_model_catalog", return_value=fake_catalog):
            validation = validate_plan(plan, privacy_policy=PrivacyPolicy("cloud_allowed"))
...
"""
    ...
    def test_full_parallel_batch_notice_is_event_not_direct_print(self):
        import runtime.execution.multi_agent_runner as runner_module
        from runtime.execution.pipeline import PipelineRunState
        ...
        events = run_state.event_bus.snapshot()
        event_types = [event.event_type for event in events]
        batch_event = next(event for event in events if event.event_type == "ParallelBatchStarted")
        self.assertIn("ParallelBatchStarted", event_types)
        self.assertEqual(batch_event.payload["task_ids"], ["read_runtime_a", "read_runtime_b"])
        self.assertEqual(batch_event.payload["batch_size"], 2)
