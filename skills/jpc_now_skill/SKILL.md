...
- 代码任务先使用 `code_locator` 定位相关文件、符号和入口，再用 `project_filesystem_readonly` 读取少量目标文件。
- 只读理解项目时使用 `project_filesystem_readonly`，但它有读取次数、文件数和字符数预算；不要一次读取大量文件。
- 需要实际创建、修改、删除文件时使用 `workspace_edit`；优先小范围 `replace_in_file` 或 `apply_unified_patch`，不要整文件覆盖，除非目标文件很小或用户明确要求。
- 修改/删除前先说明目标路径、原因、预期影响；工具会通过用户审批并在覆盖/删除前创建备份。
- 需要运行测试、lint、编译检查时使用 `command_runner`，只运行任务必要命令。
- 需要查看变更时使用 `git_tools` 的 `git_status` / `git_diff`；只有用户明确要求提交时才使用 `git_commit`。
- 不要修改 `.env`、`.git`、`.agent_quarantine`，不要访问项目根目录以外的路径。
