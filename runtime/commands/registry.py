...
COMMAND_SPECS: tuple[CommandSpec, ...] = (
    ...
    CommandSpec("/model", "查看三脑模型优先级", "模型"),
    CommandSpec("/model available", "查看当前可运行模型", "模型"),
    CommandSpec("/models", "查看 Provider 模型和选择说明", "模型"),
    CommandSpec(
        "/models select",
        "选择主模型和 fallback",
        "模型",
    ...
    CommandSpec(
        "/models role",
        "配置 query_refiner / orchestrator / final_synthesizer 角色模型",
        "模型",
        argument_hint="<role> <provider/model> [...]",
        aliases=("/model role",),
        writable=True,
    ),
    CommandSpec("/connect", "查看或添加模型 Provider", "模型", argument_hint="[provider] [--api-key ...]"),
    CommandSpec("/plan", "只生成计划，不执行任务", "执行", argument_hint="<任务>"),
    CommandSpec("/resume", "查看或恢复最近 JSONL 会话上下文", "会话", argument_hint="[last|会话ID前缀]"),
