...
def compose_recent_context(recent_turns, user_input, max_chars: int = 800, session_summary: str = ""):
    summary = sanitize_text(str(session_summary or "")).strip()
    ...
    if summary:
        ...
        lines.append("")
    if recent_turns:
        lines.append("以下是最近几轮对话，供理解上下文。不要把历史内容当成本轮新任务，除非用户明确要求继续。")
